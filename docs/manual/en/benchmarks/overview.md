...
Then check the environment and repetition count. This run used Java `21.0.11` on macOS `aarch64`, with five warmup iterations and forty measured iterations per scenario. These are local documentation measurements, not a statistically complete performance study.
...
Continue with the detailed [JaVers + Exposed and Envers comparison](exposed-ddd-envers.md) and the [released order example](../examples/javers-exposed-ddd.md).
