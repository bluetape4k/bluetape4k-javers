...
평상시 응답 속도보다 장애 뒤 어떤 질문에 답해야 하는지를 기준으로 repository를 고릅니다.
...
세 방식 모두 종단 간 exactly-once를 보장하지 않습니다. Exposed는 여러 트랜잭션으로 나뉘고, Redis는 스냅샷과 순서 구조를 따로 갱신하며, Kafka는 send 결과를 최대 30초 기다립니다. 재시도와 보정은 GlobalId, 스냅샷 버전, 커밋 ID, aggregate ID, 소비자 offset을 기준으로 설계하세요.
