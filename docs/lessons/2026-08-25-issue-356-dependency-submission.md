...
- 현재 repository API 응답에는 `security_and_analysis.dependency_graph.status` 필드가
  노출되지 않으며, `dependency-graph/sbom` endpoint는 HTTP 404를 반환한다. 두 live
  증거를 함께 확인해 현재 Dependency Graph가 비활성임을 판정한다.
...
- `push`와 `workflow_dispatch` 모두 `github.sha`를 checkout해 branch 이름이 아닌
  이벤트의 exact commit을 대상으로 삼는다. 단, checkout과 Gradle 실행은 제출 job의
  신뢰 경계 안에서만 수행한다.
...
실행하지 않고, `notice`와 `GITHUB_STEP_SUMMARY`에 의도적인 skip receipt를 남긴다. 관리자가
graph를 활성화하면 `develop` push 또는 `develop` manual dispatch에서만 동일한 workflow가
`enabled` 경로로 전환되어 의존성 입력 데이터 제출을 재개한다. 활성화 후 제출 action 자체가
실패하면 기존처럼 workflow 실패와 action 로그를 확인할 수 있다.

## 검증

- repository live read-back: `security_and_analysis.dependency_graph.status` 필드 누락과
  dependency graph SBOM endpoint HTTP 404를 각각 확인해 현재 capability가 비활성임을
  검증했다.
- `actionlint .github/workflows/dependency-submission.yml`
- workflow YAML/static contract assertions와 `git diff --check`
- hosted exact-head `workflow_dispatch` [run 32803489725](https://github.com/bluetape4k/bluetape4k-javers/actions/runs/32803489725)는
  workflow implementation parent head `f9461b1062bda01a9cf86dc9906823b8a147b496`에서 성공했다. capability job은
  `contents: read`로 SBOM HTTP 404 notice와 summary를 기록했고, feature branch의
  write 권한 `Dependency Submission` job은 skip되었다.
- 같은 exact parent head의 수동 PR CI [run 32803491392](https://github.com/bluetape4k/bluetape4k-javers/actions/runs/32803491392)도
...
누락된 status를 임의로 `disabled`로 매핑하지 않는다. write 권한이 필요한 checkout과
제출은 신뢰된 `develop` 실행에서만 수행한다. 의존성 입력 데이터 생성 성공과 원격 제출
성공을 하나의 성공 신호로 합치지 않는다. repository 보안 설정을 변경하는 작업은 별도의
권한·보안 검토와 live read-back을 거친 뒤 수행한다.
